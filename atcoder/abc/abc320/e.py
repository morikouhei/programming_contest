...
TWS = [list(map(int,input().split())) for i in range(m)]

event = []
for i,(t,w,s) in enumerate(TWS):
    event.append([t*2,i,w])
    event.append([(t+s)*2-1,m+i,0])

event_ret = [-1]*m
event.sort()

ss = SortedSet([i for i in range(n)])


ans = [0]*n

for t,ind,w in event:
    if ind < m and len(ss):
