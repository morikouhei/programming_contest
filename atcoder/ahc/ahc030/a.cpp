...
double get_normal_dist_interval_direct(int now_num, int now_res, int k) {
...
struct Environment {
    ...
    vector<vector<array<int, 2>>> ans_query_list;
    ...
    int answer_query(const vector<array<int, 2>>& ans_list) {
        ...
        for (auto asked_q : ans_query_list){
            if (asked_q == ans_list) return 0;
        }
        ...
        if (res) {
            ...
            ans_query_list.push_back(ans_list);
            cost += 1;
            return res;
        }
...
struct State {
    ...
    void solve_m2(){
        ...
        for (int i1 = 0; i1 < n2; i1++){
            ...
            for (int i2 = 0; i2 < n2; i2++){
                ...
                for (int i = 0; i < query_list.size(); ++i) {
                    ...
                    double prob = get_normal_dist_interval_direct(count,query_ans[i],query_length[i]);
                    cand_score += prob;
                }
        ...
        for (auto& [s,va] : cand){
            ...
            vector<array<int,2>> q = make_ans_query(va);

            if (ind <= 2){
                int res = environment.answer_query(q);

            } else{
                break;
            }
            // int res = environment.ask_query(q);
            // double prob = get_normal_dist_interval_direct(Block_sum,res,q.size());
    ...
    void solve_m3(){
        ...
        vector<array<int,2>> search_pos_list;
        vector<pair<double,vector< array<int,2> > > > cand;
        vector<vector<int>> state_temp(n,vector<int>(n,0));

        for (auto& [x,y] : block_pos_list){
        ...
        for (int x = 0; x < n; x++){
            for (int y = 0; y < n; y++){
                if (state_temp[x][y]){
                    search_pos_list.push_back({x,y});
                }
            }
        }

        vector<int> query_weight_temp(query_list.size(),0);

        for (auto& [x1,y1] : search_pos_list){
            if (!can_put_block(0,x1,y1)) continue;

            for (auto& [nx,ny] : Blocks[0].block_list){
                for (auto qind : query_info_list[x1+nx][y1+ny]){
                    query_weight_temp[qind]++;
                }
            }

            for (auto& [x2,y2] : search_pos_list){
   
                if (!can_put_block(1,x2,y2)) continue;

                for (auto& [nx,ny] : Blocks[1].block_list){
                    for (auto qind : query_info_list[x2+nx][y2+ny]){
                        query_weight_temp[qind]++;
                    }
                }

                for (auto& [x3,y3] : search_pos_list){
                    if (!can_put_block(2,x3,y3)) continue;

                    for (auto& [nx,ny] : Blocks[2].block_list){
                        for (auto qind : query_info_list[x3+nx][y3+ny]){
                            query_weight_temp[qind]++;
                        }
                    }
                    ...
                    // クエリリストを使ったスコアの計算
                    for (int i = 0; i < query_list.size(); ++i) {
                        double prob = get_normal_dist_interval_direct(query_weight_temp[i],query_ans[i],query_length[i]);
                        cand_score += prob;
                    }
        ...
        for (int turn = 0; turn < 2 * n2; ++turn) {
            ...
            if (timer.yet(time_early_lim)){
                for (int i = 0; i < loop_size; ++i) {
                    ...
                    if (score_dif > 0 || exp(score_dif *100/ temp)>= rng.random()) {
