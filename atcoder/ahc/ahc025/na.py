...
class Environment:
    ...
    def double_ban_apply(self,lind,rind):
    ...
    def double_ban_check(self,lind,rind):
...
class State:
    ...
    ## move one item from largest to smallest group
    def neighbor1(self):
        ...
        move_index = self.ans_group[large_group][random.randint(0,large_size-1)]

        if environment.single_ban_check(move_index):
            return 0


        nex_large_group = self.group_remove(self.ans_group[large_group],[move_index])
        nex_small_group = self.group_add(self.ans_group[small_group],[move_index])

        query = self.make_query(self.ans_group[small_group],nex_large_group)
        ...
        if res == ">":
            ...
            environment.single_ban_apply(move_index)
            return 0
        ...
        self.ans_group[small_group] = nex_small_group
        self.ans_group[large_group] = nex_large_group
    ...
    def neighbor2(self,llim=1,rlim=1):
        ...
        large_chose = random.randint(1,llim)
        small_chose = random.randint(large_chose,rlim)
        ...
        if len(move_large_index) == len(move_small_index) == 1 and environment.double_ban_check(move_small_index[0],move_large_index[0]):
            return 0


        query = self.make_query(list(move_small_index),list(move_large_index))
        ...
        if res != "<":
            if len(move_large_index) == len(move_small_index) == 1:
                environment.double_ban_apply(move_small_index[0],move_large_index[0])
            return 0
    ...
    def climbing(self):
        ...
        while environment.query_times and time.time() - stime <= TIME_LIM - 0.3:
            ...
            if loop_count - last_upd > 2*N:
                ...
                group2 = random.randint(0,group1-1)
                
                self.largest_differencing_method(group1,group2)
