...
def add_block(block,num,id,invalid_blocks,new_fixed_blocks,blocks):
    single_cand = []
...
def make_greedy_ans(new_fixed_blocks,new_Xuses,new_Yuses,new_num_block):
    ...
    if num_blocks[0] < num_blocks[1]:
        blocks[0],valid = add_block(blocks[0],dx,0,new_invalid_blocks,new_fixed_blocks,blocks)
    else:
        blocks[1],valid = add_block(blocks[1],dx,1,new_invalid_blocks,new_fixed_blocks,blocks)
...
all_loop = 0
while time.time() - stime < TIME_LIM-1:
