...
int CalcTwoPositionScoreAns(int x1, int y1, int x2, int y2) {
  int res = 0;
  rep(i, 4) {
    if (IsNG(x1, y1, i)) continue;
    int nx = x1 + dx[i];
    int ny = y1 + dy[i];
    res += (ansA[x1][y1] - ansA[nx][ny]) * (ansA[x1][y1] - ansA[nx][ny]);
  }
  rep(i, 4) {
    if (IsNG(x2, y2, i)) continue;
    int nx = x2 + dx[i];
    int ny = y2 + dy[i];
    res += (ansA[x2][y2] - ansA[nx][ny]) * (ansA[x2][y2] - ansA[nx][ny]);
  }
  return res;
}
...
int Method4_3() {
  {
    ...
    rep(i, n) {
      rep(j, n) {
        ansPosA[ansA[i][j]][0] = i;
        ansPosA[ansA[i][j]][1] = j;
      }
    }
  }

  endTime = clock();
  double start_temp = 20048;
  double end_temp = 0.1;
  ...
  rep(turn, ansSize) {
    ...
    if (rand01() >= 0){
        ...
        int ansPy = ansPosA[valP][1];
        int ansQx = ansPosA[valQ][0];
        int ansQy = ansPosA[valQ][1];
        int beforeScore = CalcTwoPositionScoreAns(ansPx, ansPy, ansQx, ansQy);
        swap(ansA[ansPx][ansPy], ansA[ansQx][ansQy]);
        int afterScore = CalcTwoPositionScoreAns(ansPx, ansPy, ansQx, ansQy);

        int tmp = beforeScore - afterScore;
        ...
        ansPosA[valP][0] = ansQx;
        ansPosA[valP][1] = ansQy;
        ansPosA[valQ][0] = ansPx;
        ansPosA[valQ][1] = ansPy;
        res++;
        } else {
        swap(ansA[ansPx][ansPy], ansA[ansQx][ansQy]);
        }
    ...
    nowPx = nowPx + dx[ansD[turn]];
    nowPy = nowPy + dy[ansD[turn]];
    nowQx = nowQx + dx[ansE[turn]];
    nowQy = nowQy + dy[ansE[turn]];
  }
  return res;
}
...
void Method4() {
  ...
  while (true) {
    ...
    ansSize = min((int)moveP.size(), n * n * 4);
  ...
  if (mode != 0) {
    cout << "loop = " << loop << ", saitaku1 = " << saitaku1
         << ",saitaku2 = " << saitaku2 << endl;
  }
}
