...
done1 = [[0]*n for i in range(4)]
done2 = [[0]*n for i in range(4)]
score = []
...
while time.time() - start <= 10:
    ...
    if abs(c1id - c2id) == 1:
        ...
        ax1,ay1 = root[id][c2id+1]

        c1x,c1y = root[id][c1id]
        c2x,c2y = root[id][c2id]


        bcost = abs(bx1-c1x)+abs(c2x-ax1) + abs(by1-c1y)+abs(c2y-ay1)
        ncost = abs(bx1-c2x)+abs(c1x-ax1) + abs(by1-c2y)+abs(c1y-ay1) 
