...
def solve_back(field,dist,dist_max,par,child):
    harvest = [[[inf]*W for i in range(H)] for j in range(T+1)]
    ...
    def get_min(x,y,d):
        ...
        for nx,ny in child[x][y]:
            if harvest[d][nx][ny] == inf:
                return inf
            harvest_min = max(harvest_min,harvest[d][nx][ny])
    ...
    for day in range(1,T+1)[::-1]:
        ...
        for d,s,ind in event_out[day]:
            ...
            # print(d,s,ind)
            _,ind,x,y = heappop(h)
            assert harvest[s][x][y] == inf
            assert pos[ind] == [-1,-1,-1]
...
def solve_front(field,dist,dist_max,par,child):
    ...
    harvest = [[[-1]*W for i in range(H)] for j in range(T+1)]
    def get_min(x,y,d):
        assert field[x][y] == 2
        ...
        for nx,ny in child[x][y]:
            if harvest[d][nx][ny] == -1:
                return inf
            harvest_min = min(harvest_min,harvest[d][nx][ny])
