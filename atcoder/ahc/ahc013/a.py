...
def calc_score(N, K, C, res: Result):
    ...
    for v in res.connects:
        ...
        if temp2[i][j] != 0 or temp2[i2][j2] != 0:
            return -1
        assert temp2[i][j] == 0
        assert temp2[i2][j2] == 0
        ni,nj = i+dx,j+dy
        while (ni,nj) != (i2,j2):
            if temp1[ni][nj] != 0 or temp2[ni][nj] != 0:
                return -1
...
# for i in range(N):
#     for j in range(N):
#         if last_vis[i][j] != -1 or C[i][j] == 0 or used_path[i][j]:
#             continue
            ...
            if 0 <= nnx < N and 0 <= nny < N and last_vis2[nnx][nny] != cash2:
                if (nnx,nny) not in waiting_set:
                    if C[nnx][nny] == 0:
                        ...
                        if uf_size_base.size(pos2ind(nnx,nny)) == 1 and used_path[nnx][nny] == 0:
                            last_vis2[nnx][nny] = cash2
                            dist_vis[nnx][nny] = dist_vis[nx][ny]+1
                            moves = []
                            x,y = nnx,nny
    ...
    while True:
        ...
        while q_first:
            ...
            for dind in range(4):
                ...
                if i2 == -1:
                    continue
                # print(nx,ny,i2,j2,"second",joint_path[i2][j2],C[i2][j2],used_path[i2][j2])
                if used_path[i2][j2]:
                    continue
                ...
                if C[i2][j2] == 0:
                    ...
                    cont = 1
                    while True:
                        connect_list = check_join_search2(i2,j2,c)
                        done = set()
                        # print(i2,j2,connect_list)
                        for nnx,nny in connect_list:
                            nind = uf_size_base.find(pos2ind(nnx,nny))
                            # print(nnx,nny,nind)
                            done.add(nind)
                        ...
                        if len(done) > 1:
                            ...
                            if i2 == -1:
                                break
                            # print(nx,ny,i2,j2,"second",joint_path[i2][j2],used_path[i2][j2])
                            if used_path[i2][j2]:
                                break
                            if C[i2][j2]:
                                cont = 0
        ...
        for _,tx,ty,connect_list in joint_path_cand:
            ...
            temp_moves += cand_move
            ttx,tty = cand_move[0][:2]
            lx,ly = cand_move[-1][2:]
            C[lx][ly],C[ttx][tty] = C[ttx][tty],C[lx][ly]

            for nx,ny in connect_list:
                # print(tx,ty,nx,ny)
                uf_size_base.union(pos2ind(nx,ny),pos2ind(tx,ty))
                # if used_path[nx][ny]:
                #     # print(tx,ty,nx,ny,"add")
                #     q_first.append([nx,ny])
...
while time.time() - stime <= 2.2:
    last_vis = [[-1]*N for i in range(N)]
    uf_size = Unionfind(N**2)
    cand = [[] for i in range(K+1)]
    for i in range(N):
        for j in range(N):
            if C[i][j] == 0 or used_path[i][j] or last_vis[i][j] != -1:
                continue
            gsum,vis_list = bfs(i,j,True)
            if gsum <= 2:
                continue
