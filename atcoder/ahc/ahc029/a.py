...
class Solver:
    ...
    def _select_action(self,now_cards,now_projects,turn_left,invest_level):
        ...
        if card_cand[2] or card_cand[3]:
            ...
            #     for i in range(self.m):
            #         min_turn = min(min_turn,self.projects[i][0] >> self.invest_level)

            #     if min_turn > 50 and self.money < 30<<self.invest_level:
            #         return (card_cand[3][0],0)
        ...
        if work_project_cand and card_cand[1]:
            ...
            if work_all_best != -1:
                ...
                if (work_all_max > 0.7 and len(work_project_cand)/self.m >= 0.5) or turn_left < 20 or (self.money < 50<<self.invest_level and work_all_max > 0.7):
        ...
        select_cand.sort()
        lim = 0.25 if self.m == 6 else 0.2
        for turn,gain,card_ind,work_ind in select_cand:

            if turn == 1:
                if gain < 3 << invest_level or gain < lim * now_cards[card_ind][1]:
                    return (card_ind,work_ind)
    ...
    def _select_next_card(self, next_cards,now_projects,turn_left,invest_level,money,playout=False):
        ...
        if cand_invest != -1 and invest_level < MAX_INVEST_LEVEL and playout == False:
            ...
            # if turn_left > 200:
            #     return cand_invest
            lim = 150 if self.k == 2 else 120

            if turn_left > lim:# and (self.money-cand_min) > 20<<self.invest_level:
                return cand_invest
        ...
        for gain,i,t,w,p in work_cards:
            ...
            if t == CardType.WORK_ALL and work_project_cand:
                ...
                if work_gain > lim and work_loss < -gain:
        ...
        if cancel_all:
            ...
            for t,w,p in self.cards:
                if t == CardType.CANCEL_ALL:
                    has_cancel_all = 1
                if t == CardType.WORK_SINGLE or t == CardType.WORK_ALL:
                    has_attack = 1

            if has_attack and has_cancel_all == 0 and self.n > 2:
                cancel_all.sort()
                return cancel_all[0][1]
