...
def check(P,base_score,XY_temp):
    score = 0
    for i in range(L):
        for j in range(L):
    ...
    for i,(x,y) in enumerate(XY):
        ...
        for j,(dx,dy) in enumerate(dxy):
            ...
            ny %= L
            temps += temp_len**j * set_temp.index(P[nx][ny])
        XY_check.append(temps)

    assert XY_temp == XY_check
    ...
    for i in range(N):
        for j in range(N):
            ...
            score += get_penalty(XY_check[i],XY_check[j])
...
# if S < 20**2:
    ...
    while True:
        ...
        if types == 1:
            ...
            for i,j in XY_use[x][y]:
                ...
                for ni in range(N):
                    if i == ni:
                        continue
                    new_score -= get_penalty(temp,XY_temp[ni]) * 2
            ...
            if new_score <= base_score:
                # print(f"# update score to {base_score} time = {time.time()-stime} dif = {base_score-new_score}")
                base_score = new_score
                last_upd = loop
        ...
        #         ni = i%L
        #         nj = j%L
        #         if (ni,nj) not in XY_climbing_list or random.randint(0,1) == 0:
        #             continue
        ...
        #                 if si == ni:
        #                     continue
        #                 base_score -= get_penalty(temp,XY_temp[ni]) * 2
