...
while time.time() - stime < 2.6:
    ...
    if upd >= 50:
        if max_score > best_score:
            ...
            best_string = ans_string[:]


        
        
        if ans_string and random.random() < 0.5:
            r = random.randint(0,len(ans_string))
            for i in range(n):
                for j in range(n):
                    if tiles[i][j] == 0:
                        x,y = i,j
            reverse_tile(x,y,ans_string[r:])
            ans_string = ans_string[:r]

        left = min(20,T-len(ans_string))
        for i in range(n):
            for j in range(n):
                if tiles[i][j] == 0:
                    x,y = i,j
